...
try:
    # Get the latest extraction record details (ID 3)
    extraction = db_manager.execute_query('SELECT * FROM bank_statement_extractions WHERE id = 3')
    if extraction:
        ext = extraction[0]
        ...
        import os
        if ext['json_file_path'] and os.path.exists(ext['json_file_path']):
            print(f'\n✅ JSON file exists at: {ext["json_file_path"]}')
        else:
            print(f'\n❌ JSON file not found at: {ext["json_file_path"]}')
